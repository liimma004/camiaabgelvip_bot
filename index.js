...
if (!token) {
    console.error('Erro: A variável de ambiente TELEGRAM_BOT_TOKEN não está definida.');
    process.exit(1); // Encerra o processo se o token estiver ausente
}

const app = express();
const port = process.env.PORT || 3000; // Usa a porta fornecida pelo Railway

// Crie o bot com webhook
const bot = new TelegramBot(token, { webHook: { port } });

// URL externo fornecido pelo Railway
const externalUrl = process.env.EXTERNAL_URL || `https://${process.env.RAILWAY_PUBLIC_DOMAIN}`; // Domínio público do Railway
...
bot.setWebHook(webhookUrl)
    ...
    .catch((err) => {
        console.error('Erro ao configurar o webhook:', err.message);
        process.exit(1); // Encerra o processo se o webhook falhar
    });
